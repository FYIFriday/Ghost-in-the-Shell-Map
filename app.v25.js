...
// External data source to override initialData on load
const JSON_PATH = 'datavs.json';
...
function EditPanel({
  ...
  const [commitMsg, setCommitMsg] = useState('Update datavs.json from app');
  ...
  const updateLegend = (category, key, updates) => {
    ...
    if (updates === null) {
      ...
      if (key === null) {
        ...
        // Check if the current value is an object or primitive
        const currentValue = newLegend[category][key];
        if (typeof currentValue === 'object' && currentValue !== null && typeof updates === 'object') {
          // For objects like lines and sects, merge the updates
          newLegend[category][key] = { ...currentValue, ...updates };
        } else {
          // For primitives like symbols and statusSymbols, replace directly
          newLegend[category][key] = updates;
        }
      }
    }
  ...
  // ---- Load external JSON once at startup (overrides initialData) ----
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await fetch(`${JSON_PATH}?v=${Date.now()}`, { cache: 'no-store', credentials: 'omit' });
        if (!res.ok) {
          console.warn(`[JSON] ${JSON_PATH} not found (${res.status}); using built-in initialData`);
          return;
        }
        const remote = await res.json();

        // normalize minimal shape (match importData behavior)
        if (!remote.canvasWidth) remote.canvasWidth = 50;
        if (!remote.canvasHeight) remote.canvasHeight = 40;
        remote.legend = remote.legend || {};
        if (!remote.legend.sects) remote.legend.sects = {};
        if (!remote.legend.statusSymbols) remote.legend.statusSymbols = {};
        remote.characters = Array.isArray(remote.characters) ? remote.characters.map((char) => ({
          ...char,
          imagePosition: char.imagePosition || 'center'
        })) : [];
        remote.connections = Array.isArray(remote.connections) ? remote.connections : [];

        setData(remote);
        console.log(`[JSON] Loaded ${JSON_PATH}`, remote);
      } catch (err) {
        console.warn(`[JSON] Failed to load ${JSON_PATH}:`, err);
      }
    })();
  }, []);
  ...
  const publishSection = React.createElement(
    ...
    React.createElement(
      ...
      'Publish (Global): Update datavs.json on GitHub'
    ),
    React.createElement(
      'div',
      ...
      React.createElement(
        ...
        React.createElement(
          ...
          React.createElement(
            'div',
            { className: 'space-y-2' },
            Object.entries(data.legend.symbols || {}).map(([symbol, meaning], index) =>
              React.createElement(
                'div',
                { key: `symbol-${index}-${symbol}`, className: 'p-2 bg-gray-800 rounded' },
                React.createElement(
                  'div',
        ...
        React.createElement(
          ...
          {
            onClick: () => {
                const newKey = `emoji_${Date.now()}`;
                const newSymbols = { ...data.legend.symbols, [newKey]: 'New Symbol' };
        ...
        React.createElement(
          ...
          React.createElement(
            'div',
            { className: 'space-y-2' },
            Object.entries(data.legend.statusSymbols || {}).map(([symbol, meaning], index) =>
              React.createElement(
                'div',
                { key: `status-${index}-${symbol}`, className: 'p-2 bg-gray-800 rounded' },
                React.createElement(
                  'div',
                  { className: 'flex items-center gap-2 mb-1' },
                  React.createElement('input', {
